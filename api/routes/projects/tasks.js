...
router.put("/update-task", async (req, res) => {
  try {
    const { task_id, title, description, execution_priority, dead_line, executorId } = req.body;
    ...
    const updatedTask = await prisma.task.update({
      ...
      data: {
        title,
        description,
        execution_priority: execution_priority !== undefined ? parseInt(execution_priority) : undefined,
  ...
  try {
    ...
    const newTask = await prisma.task.create({
      data: {
        ...
        execution_priority: execution_priority ? parseInt(execution_priority) : 0,
